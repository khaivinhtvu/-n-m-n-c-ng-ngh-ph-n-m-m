(function (jQuery, $) {
					...
					function () {
						...
						$(cols).each(
							function (key, column) {
								...
								if (containerWidth + cols.length >= colsWidth) {
									// col.length fixes width round in FF
	...
	if (1 - '\0') {
		var fixHeight = function fixHeight() {
			jQuery(
				'.bd-row-flex > [class*="col-"] > [class*="bd-layoutcolumn-"] > .bd-vertical-align-wrapper, ' +
					'[class*="bd-layoutitemsbox-"].bd-flex-wide'
			).each(function () {
	...
	jQuery(function ($) {
		...
		$(document).on('click', '[data-responsive-menu] li > a:not([data-toggle="collapse"])', function responsiveClick(e) {
			...
			if (submenu.css('visibility') === 'visible') {
				...
				itemLink.closest('li').siblings('li').find('ul').parent().removeClass('show');
				itemLink.closest('li').siblings('li').children('a').removeClass('active');
	...
	jQuery(function ($) {
		'use strict';
		window.createClickForAddToCartButton = function () {
			'use strict';
			$('.add_to_cart_button')
				.filter(function () {
		...
		window.createClickForAddToCartButton();
	});
	...
	(function ($) {
		...
		window.onEventSetProductType = function (product) {
			product.children().eq(0).attr('data-updating-content', true);
			window.createClickForAddToCartButton();
		...
		function onLoad() {
			...
			if (onLoadIntervalEffects.length) {
				onLoadIntervalEffects.each(function () {
					...
					var duration = isNaN(parseFloat(effect.attr('data-animation-duration')))
						? 0
						: parseFloat(effect.attr('data-animation-duration'));
					var delay = isNaN(parseFloat(effect.attr('data-animation-delay')))
						? 0
						: parseFloat(effect.attr('data-animation-delay'));
			...
			if (slideEffects.length) {
				...
				if (carousels.length) {
					carousels.forEach(function (_class) {
						...
						_carousel.on(
							carouselEvent + '.bs.carousel',
							eventName === 'slidein' ? animateOnSlideIn(_carousel) : animateOnSlideOut(_carousel)
						);
		...
		function getMaxDuration(effects) {
			...
			effects.each(function () {
				...
				var duration = isNaN(parseFloat(effect.attr('data-animation-duration')))
						? 0
						: parseFloat(effect.attr('data-animation-duration')),
					delay = isNaN(parseFloat(effect.attr('data-animation-delay')))
						? 0
						: parseFloat(effect.attr('data-animation-delay'));
	...
	buildTypeSelector = function (layouts, parent) {
		layouts.map(function (layout) {
			...
			jQuery(i)
				...
				.addClass('data-control-id-' + layout.iconDataId);
